...
在Linux输入长文件夹名的小技巧：在输入前面名字之后按下Tab键会自动补全
...
绝对路径都是以/开头的，相对路径以../开头

ls：列出当前目录下的文件夹，默认不会显示隐藏文件夹
...
创建包含的文件夹：mkdir -p name1/name2/name3

rmdir:删除空的文件夹，如果文件里面还有文件夹需要加 -p
...
![image-20201224210524948](https://raw.githubusercontent.com/KevinAndrewl/typora-plugins-win-img/main/typora202101/06/225826-787958.png)
...
```bash
chmod 770 filename # 让所有用户都可读写执行
```
...
- nl  显示的时候，顺道输出行号！
- more 一页一页的显示文件内容（空格翻页，回车代表向下看一行，:f 显示行号）
- less 与 more 类似，但是比 more 更好的是，他可以往前翻页！ 向下查询字符串： /要查询的字符串 向上查询：?要查询的字符串 n 继续寻找下一个 N与n寻找的方向相反
...
创建硬链接方式：ln name name

软链接：ln -s name name

### Vim

vim + filename ：新建或者修改一个文件

三种模式：命令模式、输入模式、底线命令模式

最开始进入文件时是命令模式，按下i键可进入insert模式，也就可以输入了，在输入修改完成之后，按下ESC键退出编辑模式，此时是命令模式，按下:进入底线命令模式，:wq：保存退出
...
1. 虚拟机键盘输入问题：有可能输入密码报错
2. 网络配置问题：ip与主机ip重合：使用dhclient重新获得
3. 远程访问失败：本地防火墙和Linux防火墙是否关闭
4. 阿里云的密码:jfyJFY2001
